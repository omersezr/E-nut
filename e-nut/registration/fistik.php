...
$code = $_POST['code'];
$result = mysqli_query($con,"SELECT * FROM `products3` WHERE `code`='$code'");
...
if(empty($_SESSION["shopping_cart"])) {
	...
	$status = "<div class='box'>Ürünler sepetinize eklendi!</div>";
}else{
	$array_keys = array_keys($_SESSION["shopping_cart"]);
	if(in_array($code,$array_keys)) {
		$status = "<div class='box' style='color:red;'>
		Ürün zaten sepetinizde!</div>";	
	} else {
	$_SESSION["shopping_cart"] = array_merge($_SESSION["shopping_cart"],$cartArray);
	$status = "<div class='box'>Ürün sepetinize eklendi!</div>";
	}
...
<li><a>DİL</a>
			<ul>
			<li><a href="fistik.php">Türkçe</a></li>	
		    <li><a href="fistikeng.php">English</a></li>  
	   </ul>
...
?>
<div class="cart_div">
<a href="cart.php"><img src="cart-icon.png" /> Sepet<span><?php echo $cart_count; ?></span></a>
</div>	
<?php
}

$result = mysqli_query($con,"SELECT * FROM `products3`");
while($row = mysqli_fetch_assoc($result)){
		echo "<div class='product_wrapper'>
			  ...
			  <button type='submit' class='buy'>Satın Al</button>
			  </form>
		   	  </div>";
        }
mysqli_close($con);
